...
type ServiceTagFormValues = z.infer<typeof serviceTagSchema>

interface ServiceTagFormModalProps {
    open: boolean
    onCloseAction: (open: boolean) => void
    onSubmitAction: (values: ServiceTagFormValues) => Promise<void> | void
    initialData?: ServiceTagFormValues
    loading?: boolean
}

const ServiceTagFormModal = memo(function ServiceTagFormModal({
                                                                  open,
                                                                  onCloseAction,
                                                                  onSubmitAction,
                                                                  initialData,
                                                                  loading = false,
                                                              }: ServiceTagFormModalProps) {
    const {
        register,
        handleSubmit,
    ...
    const handleFormSubmit = useCallback(async (data: ServiceTagFormValues) => {
        try {
            ...
            >
                <DialogHeader className="space-y-2">
                    <DialogTitle className="text-base sm:text-lg md:text-xl font-semibold leading-tight">
                        {isEditMode ? "Edit Service Tag" : "Create New Service Tag"}
                    </DialogTitle>
                    <DialogDescription className="text-sm sm:text-base text-gray-500">
                        {isEditMode
                            ? "Update the details of this service tag."
                            : "Create a new service tag to add to your store."}
                    </DialogDescription>
                </DialogHeader>
                ...
                >
                    <TextInputField
                        {...register("name")}
                        label="Tag Name"
                        placeholder="e.g.,Blood Pressure, Heart Rate, etc."
...
ServiceTagFormModal.displayName = "ServiceTagFormModal"

export default ServiceTagFormModal