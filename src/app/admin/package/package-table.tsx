...
export default function PackageTable() {
  ...
  const confirmDeletePackage = useCallback(async () => {
    ...
    try {
      ...
      toast.success(
        `Package "${selectedPackage.package_name}" deleted successfully`,
      );
    } catch (error: any) {
      const errorMessage =
        ...
        cell: ({ row }) => {
          return (
            <Badge
              ...
              className={cn(
                row.original.status ? "bg-green-600 " : "bg-red-500",
              )}
            >
  ...
  const renderSubComponent = useCallback((row: any) => {
    ...
    return (
      <div
        className="border-l-4 border-primary/20 bg-muted/30 p-6"
        role="region"
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Manage your packages and pricing
        </p>
      </header>
