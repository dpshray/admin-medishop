...
export default function ProductForm() {
    ...
    const categoriesOptions = categories.map((category) => ({id: category.id, name: category.name}));
    const tagsOptions = tags.map((tag) => ({id: tag.id, name: tag.name}));
    const brandsOptions = brands.map((brand) => ({id: brand.id, name: brand.name}));
    ...
    const handleBrandChange = (brandId: string | number) =>
        setValue("brand_id", typeof brandId === "string" ? parseInt(brandId) : brandId, {shouldValidate: true});
                            ...
                            <CardContent className="space-y-8">
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    <TextInputField {...register("name")} label="Product Name"
                                                    ...
                                                    required/>
                                    <SearchSelectField
                                        options={brandsOptions.map((b) => ({value: b.id, label: b.name}))}
                                        value={watchBrandId || ""}
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        <MultiSelectField
                                            options={categoriesOptions.map((c) => ({value: c.id, label: c.name}))}
