...
const DetailCard = ({ icon, label, value, color = 'blue', truncate = false }: DetailCardProps) => {
    ...
    return (
        <div className="flex items-center gap-3">
            <div className={`p-2 rounded-lg flex-shrink-0 ${colorClasses[color]}`} aria-hidden="true">
                {icon}
            </div>
            <div className="min-w-0 flex-1">
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-0.5">{label}</p>
                <p className={`text-lg font-semibold text-slate-900 dark:text-slate-100 ${truncate ? 'truncate' : ''}`}>
                    {value}
                </p>
            </div>
        </div>
...
export default function VendorProductDetails({slug}: { slug: string}) {
    ...
    const acceptMutation = useMutation({
        mutationFn: () => vendorProductService.acceptAndRejectVendorProduct(slug, true),
        onSuccess: () => {
    ...
    const rejectMutation = useMutation({
        mutationFn: () => vendorProductService.acceptAndRejectVendorProduct(slug, false),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['vendor-product', slug] })
            queryClient.invalidateQueries({ queryKey: ['vendor-products'] })
            ...
            <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
                <div className="max-w-6xl mx-auto px-4 py-6">
                    <Alert variant="destructive" role="alert">
                        <AlertDescription>{(error as any)?.message || 'Failed to load product details'}</AlertDescription>
                    </Alert>
                </div>
            </div>
        )
    ...
    if (!data || !data.product_variations || data.product_variations.length === 0) {
        return (
            <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
                <div className="max-w-6xl mx-auto px-4 py-6">
                    <Alert role="status">
    ...
    const { product_name, product_image, vendor, product_variations, is_approved } = data
                        ...
                        <div className="p-6 sm:p-8 flex flex-col">
                            <header className="flex items-start justify-between gap-4 mb-6">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div>
                                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Variation</p>
                                ...
                                {is_approved !== null && (
                                    <div className="flex flex-col items-end gap-2">
                                        ...
                                        <StatusBadge status={is_approved ? STATUS_TYPE.ACCEPTED : STATUS_TYPE.REJECTED} />
                                    </div>
                                )}
                            </header>
                            ...
                            <div className="space-y-6 flex-1">
                                ...
                                <section aria-labelledby="vendor-info-heading" className="border-t border-slate-200 dark:border-slate-800 pt-6">
                                    ...
                                    <div className="space-y-3">
                                        <DetailCard icon={<User className="w-5 h-5" />} label="Name" value={vendor.name} />
                                        <DetailCard icon={<Mail className="w-5 h-5" />} label="Email" value={vendor.email} truncate />
                                        <DetailCard icon={<Store className="w-5 h-5" />} label="Vendor ID" value={`#${vendor.id}`} />
                                    </div>
                                </section>
                            </div>
