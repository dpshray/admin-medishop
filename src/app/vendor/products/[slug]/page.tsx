...
const VariationItem = ({ variation, index }: VariationItemProps) => {
  const isLowStock =
    variation.units_in_stock < 10 && variation.units_in_stock > 0;
  const isOutOfStock = variation.units_in_stock === 0;

  return (
    <article
      className={cn(
        "group relative overflow-hidden rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 transition-all duration-300 hover:shadow-lg focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2",
      )}
...
export default function ProductPageDetail() {
  ...
  const {
    ...
    queryFn: async () =>
      vendorProductService.getVendorProductDetail(slug as string),
  ...
  const stats = useMemo(() => {
    ...
    const lowStockCount = product.variations.filter(
      (v) => v.units_in_stock < 10 && v.units_in_stock > 0,
    ).length;
    const outOfStockCount = product.variations.filter(
      (v) => v.units_in_stock === 0,
    ).length;
    const maxPossibleStock = product.variations.length * 100;
    const stockPercentage = Math.min(
      Math.round((totalStock / maxPossibleStock) * 100),
      100,
    );
    return {
      totalStock,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 xl:p-12 space-y-6 sm:space-y-8">
        ...
        <header className="space-y-4 sm:space-y-6">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 sm:gap-6">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3">
              <StatusBadge
                status={
                  product.accepted ? STATUS_TYPE.ACCEPTED : STATUS_TYPE.PENDING
                }
              />
          ...
          {stats && (stats.lowStockCount > 0 || stats.outOfStockCount > 0) && (
            <Alert
              className="border-l-4 border-l-amber-500 bg-amber-50 dark:bg-amber-900/10"
              role="alert"
            >
              <AlertCircle
              ...
              <AlertDescription className="text-sm text-amber-800 dark:text-amber-400 font-medium">
                {stats.outOfStockCount > 0 && (
                  <span>
                    {stats.outOfStockCount} variation
                    {stats.outOfStockCount > 1 ? "s" : ""} out of stock.{" "}
                  </span>
                )}
                {stats.lowStockCount > 0 && (
                  <span>
                    {stats.lowStockCount} variation
        ...
        {stats && (
          <section
            className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6"
        ...
        <section className="space-y-4 sm:space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            ...
            >
              ...
              {product.variations.length === 1 ? "Variation" : "Variations"}
            </Badge>
          </div>
