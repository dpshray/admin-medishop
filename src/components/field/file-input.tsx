...
interface FileInputFieldProps extends Omit<
...
> {
  ...
  allowedTypes?: string[];
  helperText?: string;
  // Single existing image (featured)
  existingImageUrl?: string | null;
  existingImageAlt?: string;
  onRemoveExistingImage?: () => void;
  // Multiple existing images (gallery)
  existingImages?: ExistingImage[];
  onRemoveExistingGalleryImage?: (id: number | string) => void;
}
...
const FileInputField = memo(
  ({
    ...
    // Track which new-file previews are "new" vs existing
    // For gallery mode, existing images are shown separately from new uploads
    ...
    const removeExistingImage = useCallback(() => {
      ...
      onRemoveExistingImage?.();
      onFileChange?.([]);
    }, [onFileChange, onRemoveExistingImage]);
    ...
    const hasExistingGallery = existingImages && existingImages.length > 0;
    ...
    return (
      <div className="w-full space-y-2 sm:space-y-3">
        ...
        {/* Single existing image (featured image) */}
          ...
          <div className="mb-3">
            ...
            <div className="relative inline-block group">
              <img
                ...
                className="h-16 w-16 sm:h-20 sm:w-20 max-w-full rounded-lg border-2 border-border object-contain bg-muted/30"
              />
              {!disabled && (
                <Button
                  type="button"
                ...
                <div key={img.id} className="relative group">
                  ...
                  {!disabled && (
                    <Button
                      ...
                      onClick={() => onRemoveExistingGalleryImage?.(img.id)}
        ...
        >
          <Input
            ...
            multiple={multiple}
            accept={accept}
            required={required && !showExistingImage && !hasExistingGallery}
            disabled={disabled}
        ...
        {/* New file previews (user just added) */}
        {showPreviews && previews.length > 0 && (
          <div>
            {hasExistingGallery && (
              <p className="text-xs text-muted-foreground mb-2">New Images (pending upload):</p>
            )}
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2 sm:gap-3">
              {previews.map((src, index) => (
                <div key={`preview-${index}-${src}`} className="relative group">
                  <img
                    ...
                    className="h-16 w-16 sm:h-20 sm:w-20 object-cover rounded-lg border-2 border-primary/40 border-dashed"
                    loading="lazy"
                  />
                  {!disabled && (
                    <Button
