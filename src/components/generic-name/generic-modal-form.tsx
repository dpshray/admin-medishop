...
export default function GenericModalForm({
    ...
    const fetchGenericName = useCallback(async () => {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : "Failed to fetch generic name"
            toast.error(message)
    ...
    }, [open, isEditMode, slug, initialData, reset, fetchGenericName, defaultValues])

    const handleFormSubmit = useCallback(async (data: GenericNameFormValues) => {
        try {
            if (isEditMode && slug) {
                const response = await genericNameService.updateGenericName(slug, data)
                toast.success(response?.message || "Generic name updated successfully")
            } else {
                const response = await genericNameService.createGenericName(data)
                toast.success(response?.message || "Generic name created successfully")
            }
            ...
            const message = error instanceof Error ? error.message : `Failed to ${isEditMode ? "update" : "create"} generic name`
            toast.error(message)
    ...
    const dialogTitle = isEditMode ? "Edit Generic Name" : "Create New Generic Name"
    const dialogDescription = isEditMode ? "Update the generic name details" : "Add a new generic name to the system"
    const submitLabel = isEditMode ? "Update generic name" : "Create generic name"
