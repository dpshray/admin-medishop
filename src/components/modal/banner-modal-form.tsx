...
import {bannerCreateSchema, bannerUpdateSchema} from "@/lib/schema/bannerSchema"
import {z} from "zod"
...
export function BannerFormModal({open, onOpenChange, banner, onSuccess}: BannerModalProps) {
    const [isInitializing, setIsInitializing] = useState(false)
    const [initialFormData, setInitialFormData] = useState<BannerFormData | null>(null)

    const isEditMode = Boolean(banner)
    const schema = useMemo(() => isEditMode ? bannerUpdateSchema : bannerCreateSchema, [isEditMode])
    ...
    const fetchBannerData = useCallback(async (uuid: string) => {
    ...
    const onFormSubmit = useCallback(async (data: BannerFormData) => {
        try {
                    ...
                    {isInitializing ? (
                        <div className="flex items-center justify-center py-12">
                            <div className="flex flex-col items-center gap-3">
                                ...
                                <p className="text-sm text-slate-600">Loading banner details...</p>
                            </div>
                        </div>
                    ) : (
                        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 sm:space-y-6" noValidate>
                            <div className="space-y-4">
                                ...
                                <FileInputField
                                    ...
                                    helperText={isEditMode ? "Upload a new image to replace the current one" : "Upload banner image (max 5MB)"}
                                />
                            </div>

                            <DialogFooter
                                className="flex-col-reverse sm:flex-row gap-2 sm:gap-2 pt-4 border-t mt-6 flex-shrink-0">
                                <Button
                                    type="button"
                                    variant="outline"
                                    onClick={handleClose}
