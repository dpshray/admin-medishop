...
const FORM_TYPE_OPTIONS = Object.keys(PRODUCT_FORM_DATA).map((key) => ({value: key, label: key}))
...
const ProductManageForm = ({mode = "create", productUuid, onSuccessAction}: ProductManageFormProps) => {
    ...
    const handleFormTypeChange = useCallback((index: number) => (value: string | number) => {
        setValue(`variations.${index}.variant_form_type`, value as string, {shouldValidate: true})
        setValue(`variations.${index}.variant_package_type`, "", {shouldValidate: false})
        setValue(`variations.${index}.variant_unit`, "", {shouldValidate: false})
    }, [setValue])

    const handlePackageTypeChange = useCallback((index: number) => (value: string | number) => {
        setValue(`variations.${index}.variant_package_type`, value as string, {shouldValidate: true})
    }, [setValue])

    const handleUnitChange = useCallback((index: number) => (value: string | number) => {
    ...
    const handleDeleteExistingGalleryImage = useCallback((imageId: number | string) => {
        deleteImageMutation.mutate({productUuid: productUuid ?? "", imageUuid: String(imageId)}, {
            onSuccess: () => {
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 px-4 py-6 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-7xl space-y-6">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                    <div className={cn("rounded-xl p-3 shadow-sm", isUpdateMode ? "bg-gradient-to-br from-amber-500 to-orange-500" : "bg-gradient-to-br from-blue-500 to-indigo-600")}>
                        {isUpdateMode ? <Edit className="h-6 w-6 text-white"/> : <Package className="h-6 w-6 text-white"/>}
                    </div>
                ...
                <div onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        ...
                        <div className="mt-4 grid grid-cols-1 gap-4 sm:mt-6 sm:gap-6 md:grid-cols-2">
                            ...
                            <div className="flex items-center justify-between rounded-xl border-2 border-slate-200 bg-gradient-to-br from-slate-50 to-white p-4 transition-colors hover:border-slate-300 sm:p-5">
                                <div className="space-y-0.5 sm:space-y-1">
                                    <label htmlFor="prescription-required" className="cursor-pointer text-sm font-semibold text-slate-900">
                    ...
                    <div className="rounded-2xl border border-slate-200/60 bg-white p-6 shadow-lg sm:p-8">
                        <div className="mb-6 flex items-center gap-3">
                            ...
                            <h2 className="text-lg font-semibold text-slate-900 sm:text-xl">Categories & Classification</h2>
                        </div>
                    ...
                    <div className="rounded-2xl border border-slate-200/60 bg-white p-6 shadow-lg sm:p-8">
                        ...
                        <div className="space-y-4 sm:space-y-5">
                            {fields.map((field, index) => {
                                const watchedFormType    = watch(`variations.${index}.variant_form_type`) as string
                                const watchedPackageType = watch(`variations.${index}.variant_package_type`) as string
                                const watchedUnit        = watch(`variations.${index}.variant_unit`) as string
                                const watchedBatchNo     = watch(`variations.${index}.variant_batch_no`) as string

                                const formTypeData    = PRODUCT_FORM_DATA[watchedFormType] ?? null
                                const packageTypeOpts = formTypeData?.packageTypes.map((pt) => ({value: pt, label: pt})) ?? []
                                const unitOpts        = formTypeData?.units.map((u) => ({value: u, label: u})) ?? []
                                    ...
                                    >
                                        ...
                                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-5">
                                            <SearchSelectField
                                                ...
                                                error={(errors.variations?.[index] as any)?.variant_form_type?.message}
                                                required
                                            />
                                            <SearchSelectField
                                                label="Package Type"
                                                placeholder={watchedFormType ? "Select package type" : "Select form type first"}
                                                options={packageTypeOpts}
                                                value={watchedPackageType}
                                                onChange={handlePackageTypeChange(index)}
                                                error={(errors.variations?.[index] as any)?.variant_package_type?.message}
                                                required
                                                disabled={!watchedFormType}
                                            />
                                            <SearchSelectField
                                                label="Unit"
                                                placeholder={watchedFormType ? "Select unit" : "Select form type first"}
                                                options={unitOpts}
                                                value={watchedUnit}
                                                onChange={handleUnitChange(index)}
                                                error={(errors.variations?.[index] as any)?.variant_unit?.message}
                                                required
                                                disabled={!watchedFormType}
                                            />
                                            <TextInputField
                                            ...
                                            <TextInputField
                                                ...
                                                error={(errors.variations?.[index] as any)?.variant_strength?.message}
                                                required
                                                autoComplete="off"
                                            />
                                        </div>
