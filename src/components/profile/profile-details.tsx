...
import {AlertCircle, Mail, Shield, Smartphone, User, Edit, RefreshCw, Camera} from "lucide-react"
...
const InfoCard = memo(({icon: Icon, label, value}: { icon: any; label: string; value: React.ReactNode }) => (
...
export default function ProfileDetails() {
    ...
    const statusConfig = useMemo(() => {
        ...
        return {
            ...
            bg: isActive ? 'bg-green-50 dark:bg-green-950' : 'bg-red-50 dark:bg-red-950'
        }
    }, [profile?.status])
    ...
    const handleModalChange = useCallback((open: boolean) => setIsEditModalOpen(open), [])
                        ...
                        <Card className="shadow-lg">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <InfoCard
                                    ...
                                    value={
                                        <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full ${statusConfig.bg} ${statusConfig.color} font-medium text-sm`}>
                                            <span className={`w-2 h-2 rounded-full ${profile.status === 'Active' ? 'bg-green-600' : 'bg-red-600'}`}/>
                                            {profile.status}
                                        </span>
                                    }
                                />
