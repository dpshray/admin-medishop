...
const VendorProductModal = ({
  ...
  const [addedVariations, setAddedVariations] = useState<Set<number>>(
    new Set(),
  );
  ...
  const selectedProduct = products.find(
    (product) => product.product_uuid === watchProductId,
  );
  ...
  const onSubmit = useCallback(
    async (data: VendorProductForm) => {
      try {
        ...
        const res = await vendorService.addProductByVendor(
          data.product_uuid,
          data,
        );
        if (res) {
          toast.success(
            res?.message || "Product variations updated successfully",
          );
          handleReset();
          setIsOpen(false);
        }
  ...
  const getVariationDetails = useCallback(
    (variationId: number) =>
      availableVariations.find((v) => v.id === variationId),
    [availableVariations],
  );
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <DialogDescription className="text-sm text-slate-600 mt-2">
                Streamline your product catalog with intelligent variation
                management
              </DialogDescription>
            </div>
          </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="flex flex-col gap-4">
                      <SearchSelectField
                        ...
                        onChange={(val) =>
                          setValue("product_uuid", val as string)
                        }
                    ...
                    <>
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center justify-between flex-wrap gap-3">
                          ...
                          <Badge
                            className={cn(
                              "bg-purple-100 text-purple-800 border-purple-300 font-semibold px-4 py-2",
                              { "opacity-50": !isValid },
                            )}
                          >
                            ...
                            return (
                              ...
                              >
                                ...
                                <input
                                  type="hidden"
                                  {...register(
                                    `variations.${index}.product_variation_id`,
                                    { valueAsNumber: true },
                                  )}
                                />
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                  <TextInputField
                                    ...
                                    {...register(
                                      `variations.${index}.units_in_stock`,
                                      { valueAsNumber: true },
                                    )}
                                    error={
                                      errors.variations?.[index]?.units_in_stock
                                        ?.message
                                    }
                                    type="number"
                                    min="0"
