...
export const VendorStatCard: React.FC<StatCardProps> = memo(
  ({ icon: Icon, label, value, bgColor, iconColor }) => (
    ...
    >
      ...
      <p className="text-xs text-slate-600 mb-1 font-medium tracking-wide">
        {label}
      </p>
...
export const DocumentSection: React.FC<DocumentSectionProps> = memo(
  ({ title, documents, icon: Icon }) => {
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className="text-xs">
            {documents.length > 0 ? "Available" : "Missing"}
          </Badge>
        </div>
        {documents.length > 0 ? (
...
export const InfoField: React.FC<InfoFieldProps> = memo(
  ({ label, value, className = "", copyable = false }) => {
...
export const GetTotalDocuments = (documents: VendorDocument) =>
  documents.citizenship_card.length +
  documents.tax_certificate.length +
  documents.business_license.length;
