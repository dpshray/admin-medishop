...
const BatchAllocationEditor = memo<BatchAllocationEditorProps>(function BatchAllocationEditor({
    ...
    const currentAllocationStats = useMemo(() => {
        const totalAllocated = watchedValues.reduce((sum, batch) => sum + (Number(batch.quantity) || 0), 0)
        ...
        <div className="space-y-4">
            <div
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <span className="text-xs font-medium text-gray-600 block">Total Required</span>
                        <Badge variant="outline" className="font-bold text-base mt-1 bg-white">
                            {itemQuantity}
                        </Badge>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <div>
                        <span className="text-xs font-medium text-gray-600 block text-right">Status</span>
                        <Badge
                            className={cn(
            ...
            {!currentAllocationStats.isComplete && !currentAllocationStats.hasOverAllocation && (
                                            ...
                                            <div className="flex items-center justify-between gap-4 mb-3">
                                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                                    <div className={cn(
                                                        ...
                                                        status === 'valid' && currentQuantity > 0 ? 'bg-green-500 text-white' :
                                                            status === 'invalid' ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-600'
                                                    )}>
                                                        {index + 1}
                                                    </div>
