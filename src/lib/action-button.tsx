...
const RowActions = <TData,>({
  ...
  const commonClassName =
    "w-fit justify-start rounded-xl text-xs sm:text-sm font-medium transition-all duration-200 h-auto cursor-pointer";
  return (
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                size="icon"
                className={cn(
                  "h-8 w-8 rounded-lg transition-all duration-200 hover:scale-105 hover:bg-slate-100 hover:text-slate-700 active:scale-95 dark:hover:bg-slate-800 dark:hover:text-slate-300",
                  commonClassName,
                )}
                onClick={handleView}
        ...
        {onEditAction && (
          <Tooltip>
            ...
            <TooltipContent
              side="top"
              sideOffset={5}
              className="border-[#4a358e] bg-[#4a358e] text-white "
            >
  ...
  return (
    <div className={cn("flex items-center gap-3", className)}>
      <div className="relative flex items-center">
        ...
        {isUpdating && (
          <Loader2
            className="absolute -right-7 top-1/2 h-4 w-4 -translate-y-1/2 animate-spin text-slate-400"
            aria-hidden="true"
          />
        )}
      </div>
