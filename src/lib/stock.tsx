...
const StockCell = memo<StockCellProps>(({ stock, className }) => {
    const { isOutOfStock, isLowStock } = useMemo(() => ({
        isOutOfStock: stock === OUT_OF_STOCK,
        isLowStock: stock < LOW_STOCK_THRESHOLD && stock > OUT_OF_STOCK,
    }), [stock])
        ...
        <div className={cn("flex items-center justify-center gap-2", className)}>
            {isOutOfStock ? (
                <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-sm font-semibold text-red-600 dark:text-red-400 tabular-nums" aria-label="Out of stock">
            {stock}
          </span>
                </div>
            ) : isLowStock ? (
                <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-sm font-medium text-amber-600 dark:text-amber-400 tabular-nums" aria-label={`${stock} units in stock - low stock warning`}>
            {stock}
          </span>
                </div>
            ) : (
                <span className="text-sm font-medium text-foreground tabular-nums" aria-label={`${stock} units in stock`}>
          {stock}
        </span>
            )}
        </div>
...
const StatusCell = memo<StatusCellProps>(({ status, onAcceptAction, onRejectAction, productId, isLoading = false, className }) => {
    const handleAccept = useMemo(() => () => onAcceptAction(productId), [onAcceptAction, productId])
    const handleReject = useMemo(() => () => onRejectAction(productId), [onRejectAction, productId])
        ...
        return (
            <div className={cn("flex items-center gap-2", className)}>
                ...
                <Button
                    ...
                    disabled={isLoading}
                    className={cn(
                        "h-8 text-xs font-medium bg-red-500 text-white hover:bg-red-600 hover:text-white dark:hover:bg-red-950 dark:hover:text-red-400 transition-colors disabled:opacity-50 border border-red-200 dark:border-red-800",
